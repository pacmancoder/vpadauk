...
impl<B: Bus> PdkCore<B> {
    ...
    fn execute(&mut self) -> PdkCoreState {
        ...
        let ir = self.bus.read_rom(self.pc);
        ...
        match ir.ir_opcode() {
            ...
            IrOpcode::Xorioa => self.xor_io_with_acc(ir.io_address()),
            IrOpcode::Movioa => self.bus.write_io(ir.io_address(), self.acc),
            IrOpcode::Movaio => self.alu_acc_binary(ops::mov, self.bus.read_io(ir.io_address())),
            IrOpcode::Stt16 => self.bus.write_tim16(self.bus.read_ram_word(ir.mem_address())),
            IrOpcode::Ldt16 => self.bus.write_ram_word(ir.mem_address(), self.bus.read_tim16()),
            IrOpcode::Idxmma => self.indirect_store_acc(ir.mem_address()),
            IrOpcode::Idxmam => self.indirect_load_acc(ir.mem_address()),
            IrOpcode::Retk => self.ret_immediate(ir.immediate() as u8),
            IrOpcode::T0snm => self.skip_if_bit_clear_ram(ir.mem_address(), ir.bit_index() as u8),
            IrOpcode::T1snm => self.skip_if_bit_set_ram(ir.mem_address(), ir.bit_index() as u8),
            IrOpcode::Set0m => self.clear_bit_ram(ir.mem_address(), ir.bit_index() as u8),
            IrOpcode::Set1m => self.set_bit_ram(ir.mem_address(), ir.bit_index() as u8),
            IrOpcode::Addma => self.alu_mem_binary(ops::add, ir.mem_address(), self.acc),
            IrOpcode::Subma => self.alu_mem_binary(ops::sub, ir.mem_address(), self.acc),
            IrOpcode::Addcma => self.alu_mem_binary(ops::addc, ir.mem_address(), self.acc),
            IrOpcode::Subcma => self.alu_mem_binary(ops::subc, ir.mem_address(), self.acc),
            IrOpcode::Andma => self.alu_mem_binary(ops::and, ir.mem_address(), self.acc),
            IrOpcode::Orma => self.alu_mem_binary(ops::or, ir.mem_address(), self.acc),
            IrOpcode::Xorma => self.alu_mem_binary(ops::xor, ir.mem_address(), self.acc),
            IrOpcode::Movma => self.bus.write_ram(ir.mem_address(), self.acc),
            IrOpcode::Addam => self.alu_acc_binary(ops::add, self.bus.read_ram(ir.mem_address())),
            IrOpcode::Subam => self.alu_acc_binary(ops::sub, self.bus.read_ram(ir.mem_address())),
            IrOpcode::Addcam => self.alu_acc_binary(ops::addc, self.bus.read_ram(ir.mem_address())),
            IrOpcode::Subcam => self.alu_acc_binary(ops::subc, self.bus.read_ram(ir.mem_address())),
            IrOpcode::Andam => self.alu_acc_binary(ops::and, self.bus.read_ram(ir.mem_address())),
            IrOpcode::Oram => self.alu_acc_binary(ops::or, self.bus.read_ram(ir.mem_address())),
            IrOpcode::Xoram => self.alu_acc_binary(ops::xor, self.bus.read_ram(ir.mem_address())),
            IrOpcode::Movam => self.alu_acc_binary(ops::mov, self.bus.read_ram(ir.mem_address())),
            IrOpcode::Addcm => self.alu_mem_binary(ops::addc, ir.mem_address(), self.acc),
            IrOpcode::Subcm => self.alu_mem_binary(ops::subc, ir.mem_address(), self.acc),
            IrOpcode::Izsnm => self.inc_and_skip_next_if_zero_ram(ir.mem_address()),
            IrOpcode::Dzsnm => self.dec_and_skip_next_if_zero_ram(ir.mem_address()),
            IrOpcode::Incm => self.alu_mem_binary(ops::add, ir.mem_address(), 1),
            IrOpcode::Decm => self.alu_mem_binary(ops::sub, ir.mem_address(), 1),
            IrOpcode::Clearm => self.bus.write_ram(ir.mem_address(), 0),
            IrOpcode::Xchm => self.exchange_acc_with_ram(ir.mem_address()),
            IrOpcode::Notm => self.alu_mem_unary(ops::not, ir.mem_address()),
            IrOpcode::Negm => self.alu_mem_unary(ops::neg, ir.mem_address()),
            IrOpcode::Srm => self.alu_mem_unary(ops::sr, ir.mem_address()),
            IrOpcode::Slm => self.alu_mem_unary(ops::sl, ir.mem_address()),
            IrOpcode::Srcm => self.alu_mem_unary(ops::src, ir.mem_address()),
            IrOpcode::Slcm => self.alu_mem_unary(ops::slc, ir.mem_address()),
            IrOpcode::Ceqsnam => self.skip_next_if_equal(self.bus.read_ram(ir.mem_address())),
            IrOpcode::T0snio => self.skip_if_bit_clear_io(ir.io_address(), ir.bit_index() as u8),
            IrOpcode::T1snio => self.skip_if_bit_set_io(ir.io_address(), ir.bit_index() as u8),
            IrOpcode::Set0io => self.clear_bit_io(ir.io_address(), ir.bit_index() as u8),
            IrOpcode::Set1io => self.set_bit_io(ir.io_address(), ir.bit_index() as u8),
            IrOpcode::Addak => self.alu_acc_binary(ops::add, ir.immediate() as u8),
            IrOpcode::Subak => self.alu_acc_binary(ops::sub, ir.immediate() as u8),
            IrOpcode::Ceqsnak => self.skip_next_if_equal(ir.immediate() as u8),
            IrOpcode::Andak => self.alu_acc_binary(ops::and, ir.immediate() as u8),
            IrOpcode::Orak => self.alu_acc_binary(ops::or, ir.immediate() as u8),
            IrOpcode::Xorak => self.alu_acc_binary(ops::xor, ir.immediate() as u8),
            IrOpcode::Movak => self.acc = ir.immediate() as u8,
            IrOpcode::Goto => self.goto(ir.rom_address()),
            IrOpcode::Call => self.call(ir.rom_address()),
            _ => {}
        }
    ...
    fn skip_if_bit_set_io(&mut self, addr: u8, bit: u8) {
    ...
    fn skip_if_bit_clear_io(&mut self, addr: u8, bit: u8) {
    ...
    fn skip_if_bit_set_ram(&mut self, addr: u8, bit: u8) {
    ...
    fn skip_if_bit_clear_ram(&mut self, addr: u8, bit: u8) {
        if self.bus.read_ram(addr) & (1 << bit) == 0 {
            self.pc_increment = 2;
            self.next_state = PdkCoreState::Skip;
        }
    ...
    fn load_rom_word_indirect_sp_hi(&mut self) {
        ...
        self.acc = (self.bus.read_rom(addr).original_word() >> 8) as u8
    }
    ...
    fn load_rom_word_indirect_sp_lo(&mut self) {
        ...
        let addr =
            self.bus.read_ram(sp) as u16 | ((self.bus.read_ram(sp.wrapping_add(1)) as u16) << 8);
        self.acc = self.bus.read_rom(addr).original_word() as u8
    }
