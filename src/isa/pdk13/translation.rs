...
fn pdk13_instruction_to_ir_slot(instruction: Word) -> Pdk13Result<IrSlot> {
    ...
    if instruction & MISC_GROUP_MASK == MISC_GROUP_STAMP {
        opcode_stamp = OpcodeStamp::from_primitive(instruction & MISC_GROUP_MASK);
        // No operands
    } else if instruction & XOR_IO_GROUP_MASK == XOR_IO_GROUP_STAMP {
        opcode_stamp = OpcodeStamp::from_primitive(instruction & XOR_IO_GROUP_MASK);
        ...
        opcode_stamp = OpcodeStamp::from_primitive(instruction & MOV_IO_GROUP_MASK);
        // Operand 1: 5 bit io address at offset 0
        ir_builder.operand8((instruction & 0b11111) as u8);
    } else if instruction & MEM16_GROUP_MASK == MEM16_GROUP_STAMP {
        opcode_stamp = OpcodeStamp::from_primitive(instruction & MEM16_GROUP_MASK);
        ...
        opcode_stamp = OpcodeStamp::from_primitive(instruction & RET_CONST_GROUP_MASK);
        ...
        opcode_stamp = OpcodeStamp::from_primitive(instruction & MEM_BIT_OPS_GROUP_MASK);
        ...
        opcode_stamp = OpcodeStamp::from_primitive(instruction & MEM_AND_ACC_GROUP_MASK);
        ...
        opcode_stamp = OpcodeStamp::from_primitive(instruction & MEM_GROUP_MASK);
        // Operand 1: 6 bit memory address at offset 0
        ir_builder.operand8((instruction & 0b111111) as u8);
    } else if instruction & IO_BIT_OPS_GROUP_MASK == IO_BIT_OPS_GROUP_STAMP {
        opcode_stamp = OpcodeStamp::from_primitive(instruction & IO_BIT_OPS_GROUP_MASK);
        ...
        opcode_stamp = OpcodeStamp::from_primitive(instruction & ACC_CONST_GROUP_MASK);
        // Operand 1: 8 bit immediate at offset 0
        // Convention: always place immediate to u16 ir operand
        ir_builder.operand16((instruction & 0b11111111) as u16);
    } else if instruction & JUMP_GROUP_MASK == JUMP_GROUP_STAMP {
        opcode_stamp = OpcodeStamp::from_primitive(instruction & JUMP_GROUP_MASK);
